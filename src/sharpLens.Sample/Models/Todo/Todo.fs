...
type Todo =
    { TodoId: TodoId
      Text: string
      State: State
      Comments: List<DateTime * string> }

    static member New text =
        { TodoId = TodoId.New
          Text = text
          State = Added DateTime.Now
          Comments = [] }
    ...
    member this.Activate(dateTime: DateTime) =
        result {
            ...
            return { this with State = Started dateTime }
        }
    ...
    member this.Complete dateTime =
        result {
            ...
            return { this with State = Completed dateTime }
        }

    member this.Comment(dateTime: DateTime, comment: string) =
        result {
            return
                { this with
                    Comments = this.Comments @ [ dateTime, comment ] }
        }
